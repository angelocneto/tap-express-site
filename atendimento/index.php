...
<?php else: ?>
  ...
  <?php } elseif ($view === 'usuarios'): ?>
    ...
    <div class="card"><h2>Novo usuário</h2><form method="post"><input type="hidden" name="action" value="novo_usuario"/><input type="hidden" name="csrf" value="<?= $csrf ?>"/><label>Nome</label><input name="nome" required/><label>E-mail</label><input type="email" name="email" required/><label>Senha provisória (mín. 8)</label><div class="pw"><input type="password" name="senha" minlength="8" required/><button type="button" class="eye" aria-label="Mostrar ou ocultar a senha" data-eye><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path class="e-open" d="M1.5 12s3.8-7 10.5-7 10.5 7 10.5 7-3.8 7-10.5 7S1.5 12 1.5 12z"/><circle class="e-open" cx="12" cy="12" r="3"/><path class="e-shut" d="M3 3l18 18" style="display:none"/></svg></button></div><div style="margin-top:14px"><button class="btn p" type="submit">Criar usuário</button></div></form></div></div>
  <?php else:
