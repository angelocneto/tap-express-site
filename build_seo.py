...
def unit_ld(u):
    d={"@type":["LocalBusiness","MovingCompany"],"@id":f"{BASE}/unidades/{u['slug']}/#local","name":f"TAP Express · {u['n']}","parentOrganization":{"@id":BASE+"/#organization"},"url":f"{BASE}/unidades/{u['slug']}/","telephone":u['tel'],"email":u['email'],"address":{"@type":"PostalAddress","streetAddress":u['addr'],"addressLocality":u['n'],"addressRegion":u['uf'],"postalCode":u.get('cep',''),"addressCountry":"BR"},"geo":{"@type":"GeoCoordinates","latitude":u['c'][1],"longitude":u['c'][0]},"areaServed":[{"@type":"City","name":c['n']} for c in u['cities']]+[{"@type":"City","name":u['n']}],"priceRange":"$$"}
...
faq={"@context":"https://schema.org","@type":"FAQPage","mainEntity":[
 ...
 {"@type":"Question","name":"Como rastrear uma encomenda da TAP Express?","acceptedAnswer":{"@type":"Answer","text":"O rastreamento é feito no próprio site, em https://www.tapexpress.com.br/rastreamento/, com três dados: CNPJ ou CPF do remetente, número da nota fiscal e senha de rastreio fornecida pela TAP. Os dados vêm do portal SSW. Todos os veículos têm rastreamento via satélite."}},
...
for u in units:
    ...
    page=(tpl.replace('{{TITLE}}',esc(title)).replace('{{DESC}}',esc(desc)).replace('{{URL}}',f"{BASE}/unidades/{u['slug']}/").replace('{{NAME}}',esc(u['n'])).replace('{{UF}}',u['uf'])
        .replace('{{ADDR}}',esc(u['addr'])).replace('{{NAV}}',go_links(u)).replace('{{PHONE}}',esc(u['phone'])).replace('{{TEL}}',u['tel']).replace('{{EMAIL}}',u['email']).replace('{{WA}}',u.get('wa') or wa)
...
- WhatsApp central: +{wa}
- E-mail: {rede['email']}
- Rastreamento: {BASE}/rastreamento/ (dados do portal SSW)
