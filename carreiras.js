...
(function () {
  ...
  fetch("/api/vagas.php").then(r => r.json()).then(j => {
    const v = (j && j.vagas) || [];
    if (!v.length) { list.innerHTML = '<div class="tile" style="grid-column:1/-1"><h3>' + t("Nenhuma vaga aberta neste momento") + '</h3><p>' + t("Deixe seu currículo no banco de talentos abaixo. Quando abrir uma vaga na sua região, a equipe entra em contato.") + '</p></div>'; return; }
    list.innerHTML = v.map(x => `<article class="vaga tile" id="vaga-${x.id}"><small>${esc(x.area)}${x.tipo ? " · " + esc(x.tipo) : ""}</small><h3>${esc(x.titulo)}</h3><p class="loc">${esc(x.unidade || x.cidade)}${x.uf ? " · " + esc(x.uf) : ""}</p>${x.descricao ? `<p>${esc(x.descricao)}</p>` : ""}${lines(x.requisitos).length ? `<h4>${t("Requisitos")}</h4><ul>${lines(x.requisitos).map(l => `<li>${esc(l)}</li>`).join("")}</ul>` : ""}${lines(x.beneficios).length ? `<h4>${t("Benefícios")}</h4><ul>${lines(x.beneficios).map(l => `<li>${esc(l)}</li>`).join("")}</ul>` : ""}<a class="btn btn-solid" href="#candidatura" data-vaga="${x.id}">${t("Candidatar-se")}</a></article>`).join("");
  ...
  }).catch(() => { list.innerHTML = '<div class="tile" style="grid-column:1/-1"><h3>' + t("Não foi possível carregar as vagas agora") + '</h3><p>' + t("Envie seu currículo pelo formulário abaixo.") + '</p></div>'; });
  ...
  form.addEventListener("submit", async (e) => {
    ...
    const btn = form.querySelector("button[type=submit]"); btn.disabled = true; btn.textContent = t("Enviando…");
    try {
      const r = await fetch("/api/candidatura.php", { method: "POST", body: new FormData(form) });
      const j = await r.json(); if (!r.ok || !j.ok) throw new Error(t(j.erro || "Falha ao enviar"));
      form.hidden = true; done.hidden = false; done.scrollIntoView({ behavior: "smooth", block: "center" });
    } catch (err) { msg.textContent = err.message; btn.disabled = false; btn.textContent = t("Enviar candidatura"); }
  });
})();
