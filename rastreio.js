...
(function () {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (mode === "danfe" && body.danfe.length !== 44) { show(`<div class="track-msg err">${t("A chave da NF-e tem 44 números. Você digitou {n}.").replace("{n}", body.danfe.length)}</div>`); return; }
    if (mode === "chave" && (body.cnpj.replace(/\D/g, "").length !== 14 || !body.chave)) { show('<div class="track-msg err">' + t("Informe o CNPJ completo e a chave de rastreio.") + '</div>'); return; }
    if (mode === "senha" && (body.cnpj.replace(/\D/g, "").length !== 14 || !body.senha)) { show('<div class="track-msg err">' + t("Informe o CNPJ do remetente e a senha.") + '</div>'); return; }
    ...
    try {
      ...
      if (j.encomendas && j.encomendas.length) {
        ...
        html += `<p class="track-total">${t(j.total > 1 ? "{n} encomendas nos últimos 30 dias" : "{n} encomenda nos últimos 30 dias").replace("{n}", j.total)}</p><div class="ship-list">` + j.encomendas.map((e, i) => `<article class="ship ${st(e.situacao)}" data-i="${i}"><header><div><b>${esc(e.situacao || t("Sem situação"))}</b><time>${esc(e.data)}${e.hora ? " · " + e.hora : ""}${e.unidade ? " · " + esc(e.unidade) : ""}</time></div>${e.link ? `<button type="button" class="ship-more" data-link="${esc(e.link)}">${t("Ver histórico")}</button>` : ""}</header><div class="ship-grid">${e.destinatario ? `<div><small>${t("Destinatário")}</small>${esc(e.destinatario)}</div>` : ""}${e.destino ? `<div><small>${t("Destino")}</small>${esc(e.destino)}</div>` : ""}${e.nf ? `<div><small>${t("Nota / coleta")}</small>${esc(e.nf)}</div>` : ""}${e.ctrc ? `<div><small>CTRC</small>${esc(e.ctrc)}</div>` : ""}${e.previsao ? `<div><small>${t("Previsão de entrega")}</small>${esc(e.previsao)}</div>` : ""}${e.entrega ? `<div><small>${t("Entregue em")}</small>${esc(e.entrega)}</div>` : ""}</div>${e.detalhe ? `<p>${esc(e.detalhe)}</p>` : ""}<div class="ship-hist" hidden></div></article>`).join("") + "</div>";
      }
      if (!j.ok && !(j.eventos && j.eventos.length) && !(j.encomendas && j.encomendas.length)) html += `<div class="track-msg err">${esc(t(j.motivo || "Não encontramos essa encomenda."))} ${t("Confira os dados ou fale com a unidade pelo WhatsApp.")}</div>`;
      if (j.html) html += `<details class="track-raw"${j.eventos && j.eventos.length ? "" : " open"}><summary>${t("Resposta completa do portal")}</summary><div class="track-raw-body">${j.html}</div></details>`;
      ...
      const ctx = mode === "danfe" ? "NF-e " + body.danfe : (mode === "senha" ? "Remetente " + body.cnpj : "CNPJ " + body.cnpj + " chave " + body.chave); /* contexto interno fica em PT */
  ...
  function leadBlock(ctx) {
    return `<aside class="lead-box"><h3>${t("Quer receber as atualizações desta entrega?")}</h3><p>${t("Deixe seu contato e a TAP avisa por WhatsApp ou e‑mail a cada nova etapa.")}</p>
      <form id="leadForm" class="lead-form" autocomplete="on"><input type="hidden" name="origem" value="rastreamento"/><input type="hidden" name="contexto" value="${esc(ctx)}"/><input type="text" name="website" tabindex="-1" autocomplete="off" style="position:absolute;left:-9999px" aria-hidden="true"/>
        ...
        <label class="lead-check"><input type="checkbox" name="aceite" value="1" required/> <span>${t("Autorizo a TAP Express a me enviar atualizações desta encomenda e comunicações por WhatsApp e e‑mail. Li a <0>política de privacidade</0>.").replace("<0>", '<a href="' + (window.TAP_LANG ? "/" + window.TAP_LANG : "") + '/privacidade/" target="_blank" rel="noopener">').replace("</0>", "</a>")}</span></label>
