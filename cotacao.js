...
(function () {
  ...
  chatBody.addEventListener("click", (e) => {
    ...
    if (a === "rastrear") { say("Quero rastrear uma encomenda.", true); say("Você rastreia aqui no site, com a chave da nota fiscal ou o CNPJ e a chave de rastreio. Abrindo a página…"); setTimeout(() => { toggleChat(false); location.href = "/rastreamento/"; }, 900); return; }
  ...
  chatForm.addEventListener("submit", (e) => {
    ...
    if (/rastre|onde esta|cadê|cade|encomenda|nota/.test(n)) { say("Para rastrear, use o portal oficial da TAP. Abrindo em nova aba…"); setTimeout(() => { location.href = "/rastreamento/"; }, 800); return; }
    ...
    if (/mudanca|mudança|mudar de casa|residencial/.test(n)) { say("A TAP não faz mudanças. Trabalhamos com encomendas, malotes e cargas fracionadas entre as cidades da rede. Se for uma carga desse tipo, posso cotar agora."); return; }
    if (/trabalh|curricul|vaga|emprego|contrat|rh\b/.test(n)) { say("Que bom! As vagas abertas ficam na página Trabalhe conosco, e o currículo vai direto para a nossa central de candidatos. Abrindo…"); setTimeout(() => { location.href = "/carreiras/"; }, 900); return; }
    if (/pagador|tomador|reverter|quem paga|inverter/.test(n)) { say("Trocar quem paga o frete só é possível enquanto ele ainda não foi pago. O tomador atual precisa registrar a manifestação de desacordo no prazo da SEFAZ. A unidade que emitiu o CT-e orienta o procedimento. Quer que eu te leve ao WhatsApp dela?"); return; }
    if (/prazo|quanto tempo|demora|chega|dias?\b|horas?\b/.test(n)) { say("O prazo é de até 18 horas para as cidades atendidas. Algumas cidades menores têm dia fixo de entrega, como terça e quinta. Os dias aparecem ao lado de cada cidade em <a href=\"/cidades/\">cidades atendidas</a>."); return; }
